...
const FeaturedProducts = ({ addToCart }) => {
  ...
  const handleAddToCart = (product) => {
    addToCart(product); // Only add product to cart, no navigation
  };
    ...
    <section className="py-16 bg-gray-100 shadow-md font-serif">
      <div className="container mx-auto px-4 lg:px-16">
        <div className="flex justify-between items-center mb-8">
          ...
          <Link to="/cart" aria-label="Go to Cart" className="text-lg text-gray-800 hover:text-gray-600 transition duration-300 flex items-center">
            <span className="material-icons">shopping_bag</span> {/* Shopping bag icon */}
          </Link>
        </div>
